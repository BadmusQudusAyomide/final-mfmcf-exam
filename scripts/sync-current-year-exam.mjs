...
async function main() {
  if (!fs.existsSync(questionsPath)) {
    throw new Error("current-year-questions.json does not exist.");
  }

  const questions = JSON.parse(fs.readFileSync(questionsPath, "utf8"));

  if (!Array.isArray(questions) || questions.length === 0) {
    throw new Error("No questions found in current-year-questions.json.");
  }
  ...
  console.log(`Synced ${questions.length} current-year questions into the active exam.`);
}
